...
#Update history  charts - demography and lifestyles
def update_history_charts_DELETE(filtered_df, selected_poi):
...
def update_history_charts(filtered_df, selected_poi):
    ...
    #print(long_format_lifestyle_df)
...
#Update surface charts for each POI - demography and lifestyles
...
# Get the trigger that caused the callback
    ...
    selected_poi_details = f"{poi_details['poi_name'].iloc[0]}: {poi_details['poi_category'].iloc[0]}. Located at Lat: {poi_details['poi_latitude'].iloc[0]} Long: {poi_details['poi_longitude'].iloc[0]}"
...
if __name__ == "__main__":
    ...
    #thread.start()
    try:
        print("Running Dash app4. Press Ctrl+C to exit.")
